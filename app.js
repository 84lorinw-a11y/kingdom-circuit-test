...
function renderArtistDirectory() {
  ...
  grid.innerHTML=enabled.map(a=>{const ev=byArtist.get(normalize(a.name))||[];return `<article class="artist-card artist-card-text" data-artist-card data-search="${esc(normalize([a.name,...(a.aliases||[]),a.label].join(" ")))}" data-has-shows="${ev.length>0}"><a class="artist-visual artist-visual-empty" href="${artistProfileUrl(a.name)}" aria-label="View ${esc(a.name)}"></a><div class="artist-card-body"><p class="artist-category">${esc(a.label||friendlyCategory(a.category))}</p><h2><a href="${artistProfileUrl(a.name)}">${esc(a.name)}</a></h2><p>${ev.length} upcoming show${ev.length===1?"":"s"}</p><a class="text-link" href="${artistProfileUrl(a.name)}">View artist</a></div></article>`;}).join("");
