...
function eventCard(event) {
  const search = [event.title, event.venue, event.city, event.state, ...(event.artists || [])].join(" ").toLocaleLowerCase();
  ...
  return `<article class="event-card" data-event-card data-search="${esc(search)}" data-artists="${esc(artists)}" data-state="${esc(event.state || "")}" data-type="${esc(event.eventType || "concert")}" data-date="${esc(event.startDate || "")}" data-end-date="${esc(event.endDate || event.startDate || "")}">
    ...
    <div class="event-content"><div class="event-main"><div class="event-badges"><span class="badge badge-gold">${esc(eventTypeLabel(event))}</span>${recent}</div><h3><a href="${eventDetailUrl(event)}">${esc(event.title)}</a></h3><p class="artist-line">${artistLinks(event)}</p><dl class="event-meta"><div><dt>Date</dt><dd>${esc(formatDate(event))}</dd></div><div><dt>Venue</dt><dd>${esc(event.venue || "Venue to be announced")}</dd></div><div><dt>Location</dt><dd>${esc(location)}</dd></div></dl>${price}</div><div class="event-footer"><a class="official-button" href="${esc(event.officialUrl || event.ticketUrl || "#")}" target="_blank" rel="noopener">Official details</a></div></div>
  </article>`;
}
...
function ensureArtistSuggestionDialog() {
  ...
  form?.addEventListener("submit", async event => {
    ...
    try {
      ...
      feedback.textContent = "Artist submitted. We will verify the official accounts and listings before adding them.";
...
Confirm final information with the official organizer or ticket provider before purchasing or traveling.</p></div></article>`;
  ...
  setMetaDescription(`${event.title} in ${locationText || "the United States"}. View verified event details and the official listing.`);
}
...
async function boot() {
  ...
  const staticExperience = document.querySelector(
